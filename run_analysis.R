...
download.file(fileUrl, destfile = "./data/GACDdataset.zip", mode = "wb")
unzip("./data/GACDdataset.zip", exdir = "./data")
...
features <- read.table("./data/UCI HAR Dataset/features.txt") #get names of observations in X_train and X_test datasets
...
#consolidate the test & train data sets to prepare for data merge
...
activities <- read.table("./data/UCI HAR Dataset/activity_labels.txt") # create table showing activity labels/activity pairs
